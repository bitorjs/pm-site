...
exports.getModuleTotal = async (name, start, end) => {
...
exports.getTotalByName = async (name) => {
  var rows = await DownloadTotal.findAll({
    where: {
      name: name
    }
...
exports.plusModuleTotal = async (data) => {
  var yearMonth = parseYearMonth(data.date);
  var row = await DownloadTotal.find({
    where: {
      name: data.name,
      date: yearMonth,
    }
...
exports.getTotal = async (start, end) => {
  return await exports.getModuleTotal('__all__', start, end);
};
